...
MSG_ERR_PROMPT_TEMPLATE_MISSING="❌ 未找到提示词模板 meeting-notes.md（已查找会话级/用户级/默认级路径），无法生成纪要"
MSG_WARN_PROMPT_TEMPLATE_INVALID="⚠️ 自定义模板 %s 缺少必需占位符 %s，已跳过，回退到默认模板"
MSG_ERR_PROMPT_TEMPLATE_INVALID="❌ 默认提示词模板 %s 缺少必需占位符 %s，无法生成纪要"
MSG_NOTES_DONE="✅ Meeting notes generated: %s"
MSG_NOTES_NAMED="📄 智能命名纪要副本已生成: %s"
...
MSG_FIELD_STOP_MANUAL="   自动停止: 关闭（手动 stop 结束）"
MSG_FIELD_STOP_CMD="   停止录制: %s stop"
MSG_FIELD_MAX_DURATION="   自动停止: %s 分钟后（约 %s）"
MSG_ERR_FFMPEG_FAIL="❌ ffmpeg 启动失败:"
MSG_TAP_STALL_RETRY="⚠ 系统音频采集未出数据（Process Tap 回调未启动），正在重启音频链路（第 %s/%s 次）..."
MSG_ERR_TAP_STALL="❌ 系统音频采集反复无数据，录音会是静音。请重跑一次 %s setup 检查授权，或重新执行 start。"
...
# ── Process Tap 授权检测 ──
MSG_SETUP_TAP_UNSUPPORTED="ℹ️  本机不支持 Process Tap（需 macOS 14.4+），将使用 BlackHole 录制"
MSG_SETUP_TAP_PROBING="🔍 正在检测系统音频捕获（约 3 秒，会播放一声提示音）..."
MSG_SETUP_TAP_OK="✅ 系统音频捕获正常，录制将使用 Process Tap（不切换输出设备）"
MSG_SETUP_TAP_SILENT_HEADER="⚠️  捕获到的音频全为静音 —— 终端 App 缺少系统录音授权"
MSG_SETUP_TAP_SILENT_WHY="   （部分终端如 Ghostty 不会弹出授权框，需要手动添加）"
MSG_SETUP_TAP_GUIDE_1="   1. 已为你打开: 系统设置 > 隐私与安全性 > 屏幕与系统录音"
MSG_SETUP_TAP_GUIDE_2="   2. 在「仅系统录音」中点 + 添加你的终端 App（如 Ghostty / Terminal / iTerm）"
...
MSG_SETUP_STEP_TAP="【1/4】检测系统音频捕获授权 (Process Tap)"
MSG_SETUP_STEP_ZOOM="【2/4】配置 Zoom"
...
MSG_AGAIN_EXTRA_PROMPT="   本次补充要求: %s"
...
MSG_USAGE_AGAIN="  again [目录名] [补充要求] 重新生成会议纪要（默认最近一次；补充要求直接作为额外 prompt 传给 LLM，也可放资料到 materials/）"
MSG_USAGE_HIGHLIGHT="  mark [dir]       给纪要要点加高亮标记（默认最新会话）"
