...
模型选型经验：会议纪要属于"整理归纳"任务，Sonnet 质量足够且成本仅为 Opus 的 1/5。Opus 4.7 曾测试通过但性价比不高。注意 Bedrock inference profile ID 格式：`us.anthropic.claude-sonnet-4-6` 可用，带 `-v1` 后缀的会报 "model identifier invalid"。
...
| 2026-04-25 | 模型改回 Claude Sonnet 4.6（性价比更优，成本降低 80%） |
